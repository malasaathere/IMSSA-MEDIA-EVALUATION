...
export function AdminWorkspace() {
  ...
  const { data: managedEvents = [] } = useQuery<any[]>({ queryKey: ['adminEvents'], queryFn: api.getAdminEvents })
  ...
  const eventOptions = useMemo(() => Array.from(new Set([
    ...
    ...(plansResponse?.documents || []).map((plan: any) => plan.campaign || plan.eventName || plan.eventId),
  ].filter(Boolean))).sort() as string[], [managedEvents, plansResponse]);
    ...
    <div className="min-h-screen bg-slate-50 p-4 sm:p-6 lg:p-8">
      <div className="page-heading mb-8">
        ...
        <div className="flex flex-wrap gap-2">
          <AddUserDialog eventOptions={eventOptions} onCreated={() => queryClient.invalidateQueries({ queryKey: ['adminUsers'] })} />
          <AddEventDialog users={users} onCreated={() => Promise.all([queryClient.invalidateQueries({ queryKey: ['adminEvents'] }), queryClient.invalidateQueries({ queryKey: ['adminUsers'] })])} />
